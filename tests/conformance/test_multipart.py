...
def _check_invariants_after_restart(
    ...
    # For multipart objects the ETag is a composite (md5-N), not a plain MD5 of the body —
    # checking ETag == body MD5 is only valid for single-part objects.
    etag = resp.headers.get("etag")
    assert etag is not None and len(etag) > 0, "ETag header missing after crash+restart"
